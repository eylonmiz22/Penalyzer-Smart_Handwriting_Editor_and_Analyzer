...
def collate(batch):
    ...
    return toRet
class AuthorRIMESDataset(Dataset):
    def __init__(self, dirPath, split, config):
        ...
        words_file = os.path.join(dirPath,'groundtruth_{}2009_pageNorm.txt'.format(split))
        if not os.path.exists(words_file):
            ...
            with open(os.path.join(dirPath,'groundtruth_{}2009.txt'.format(split))) as f:
                word_list = f.readlines()
            for line in word_list:
                m=re.match('(lot_.+\/([^/]+)\/[^/]+.tiff) (.+)',line.strip())
            ...
            for author,lines in authors.items():
                ...
                for i,(path,gt) in  enumerate(lines):
                    ...
                    if img is None:
                        continue
                    img = 255-img
                    img = cv2.blur(img, (21,21),borderType=cv2.BORDER_CONSTANT)#borderValue=0)
                    #th,binarized = cv2.threshold(img,0,1,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
                    #binarized = 1-binarized
                    img = img.sum(axis=1)
                    centerline = np.argmax(img) #assume center is where most ink is (after blur)
        ...
        self.max_char_len=0
        for line in word_list:
            m=re.match('(lot_.+\/([^/]+)\/[^/]+.tiff) ([^ ]+) (-?\d+) (-?\d+)',line.strip())
            path = os.path.join(dirPath,subdir,m[1])
            author = m[2]
            gt = m[3]
