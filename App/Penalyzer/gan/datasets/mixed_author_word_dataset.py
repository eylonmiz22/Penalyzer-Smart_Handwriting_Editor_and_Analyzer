...
class MixedAuthorWordDataset(Dataset):
    def __init__(self, dirPath, split, config):
        ...
        for word_idx, word_id in enumerate(set_list):
            ...
            if page_id not in allLines:
                w_lines,lines,author = parseXML(os.path.join(dirPath,'xmls',page_id+'.xml'))
                for w_line in w_lines:
                    for bounds,trans,id in w_line:
                        allLines[page_id][id] = (bounds,trans)
            bounds,trans = allLines[page_id][word_id]
            self.w_authors[author].append((os.path.join(dirPath,'forms',page_id+'.png'), bounds, trans,word_id))
