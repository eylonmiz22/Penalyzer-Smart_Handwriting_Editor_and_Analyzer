...
class CRNNSkip(BaseModel):#(nn.Module):

    def __init__(self, nclass, nc=1, cnnOutSize=512, nh=512, n_rnn=2, leakyRelu=False, norm='batch',use_softmax=True,small=False,pad=False):
        super(CRNNSkip, self).__init__(None)
        ...
        self.skip_emb = nn.Linear(nm[6],nh*2)
        self.rnn = nn.LSTM(cnnOutSize, nh, bidirectional=True, dropout=0.5, num_layers=2)
        self.embedding = nn.Sequential(nn.Linear(nh * 2, nh), nn.ReLU(True),  nn.Linear(nh, nclass))
    ...
    def forward(self, input, style=None):
        ...
        skipping = self.skip_emb(conv)
        t_rec = t_rec+skipping.view(T * b, h)
